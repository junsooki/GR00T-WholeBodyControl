 ...
 * ## Robot support
 *
 * The planner is currently **G1-only**.  Its context and output buffers are
 * addressed at a stride of `(NUM_MOTOR + 7)` -- 7 root DOF (xyz position plus
 * a wxyz quaternion) followed by one value per motor.  That stride describes
 * the *checkpoint's* flattened MuJoCo qpos layout, which is baked into the
 * trained model, not something the robot can renegotiate: every released
 * planner emits 36 floats per frame (29 G1 motors + 7).  On an H2 build the
 * expression evaluates to 38, which no released model produces, so
 * `ROBOT_HAS_PLANNER` gates the planner off there -- see the
 * `LocalMotionPlannerBase` constructor, which refuses to build one.
 */
...
public:
     ...
     * @throws std::runtime_error on a robot with no planner checkpoint (H2)
     */
    LocalMotionPlannerBase(const PlannerConfig& config = PlannerConfig())
        : config_(config),
          current_random_seed_(config.initial_random_seed)
    {
        // No planner checkpoint exists for this robot. A planner is only ever
        // constructed because the operator passed --planner-model, so refuse
        // loudly here rather than let a G1 checkpoint's 36-wide qpos frames be
        // resampled into this robot's joint slots and driven at 500 Hz.
        if constexpr (!ROBOT_HAS_PLANNER) {
            std::cout << "✗ Error: no locomotion planner exists for '" << ROBOT_NAME << "'" << std::endl;
            throw std::runtime_error(
                std::string("Locomotion planner requested on a '") + ROBOT_NAME +
                "' build (model: " + (config.model_path.empty() ? "<none>" : config.model_path) +
                "), but no '" + ROBOT_NAME + "' planner checkpoint exists. This planner reads " +
                "(NUM_MOTOR + 7) = " + std::to_string(NUM_MOTOR + 7) +
                " floats per qpos frame; every released model emits 36 (G1). " +
                "Run without --planner-model, or build for g1 (drop -DROBOT_H2=1).");
        }

        planner_motion_50hz_.ReserveCapacity(1500, NUM_MOTOR, 1, 1, 0, 0);
        gen_frame_ = 0;
...
public:
    ...
    bool Initialize(
        ...
        const std::array<double, NUM_MOTOR>& joint_positions) {
    ...
    bool ResampleGeneratedSequence50Hz()
    {
        ...
        if(timesteps_30hz > 64)
        {
            ...
            /*
            for(int i = 0; i < (NUM_MOTOR + 7) * timesteps_30hz; i++)
            {
            ...
            cnpy::npy_save("context_qpos.npy", GetContextBuffer(), std::vector<size_t>{4, (NUM_MOTOR + 7)}, "w");
            */
        ...
        if(
            std::any_of(
                mujoco_qpos_data, mujoco_qpos_data + (NUM_MOTOR + 7) * timesteps_30hz,
        ...
        for(int32_t f_50hz = 0; f_50hz < planner_motion_50hz_.timesteps; ++f_50hz)
        {
            ...
            planner_motion_50hz_.BodyPositions(f_50hz)[0] = std::array<double, 3> {
                w0 * mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 0] + w1 * mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 0],
                w0 * mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 1] + w1 * mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 1],
                w0 * mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 2] + w1 * mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 2]
            };
            
            // resample global body quaternion:
            std::array<double, 4> q0 = {
                mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 3],
                mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 4],
                mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 5],
                mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 6]
            };
            std::array<double, 4> q1 = {
                mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 3],
                mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 4],
                mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 5],
                mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 6]
            };
            planner_motion_50hz_.BodyQuaternions(f_50hz)[0] = quat_slerp_d(q0, q1, f_30hz - f0);

            // resample joint angles:
            for(int joint = 0; joint < NUM_MOTOR; ++joint) {
                planner_motion_50hz_.JointPositions(f_50hz)[joint] =
                    w0 * mujoco_qpos_data[f0 * (NUM_MOTOR + 7) + 7 + mujoco_to_isaaclab[joint]] + 
                    w1 * mujoco_qpos_data[f1 * (NUM_MOTOR + 7) + 7 + mujoco_to_isaaclab[joint]];
            }
        }
    ...
    void UpdateContextFromMotion(const std::shared_ptr<const MotionSequence>& motion_sequence) {
        ...
        for(int n=0; n<4; ++n)
        {
            ...
            std::copy(quat.begin(), quat.end(), &context_qpos_values[n * (NUM_MOTOR + 7) + 3]);
            ...
            context_qpos_values[n * (NUM_MOTOR + 7) + 0] = p[0];
            context_qpos_values[n * (NUM_MOTOR + 7) + 1] = p[1];
            context_qpos_values[n * (NUM_MOTOR + 7) + 2] = p[2];

            // resample joint positions
            for(int i = 0; i < NUM_MOTOR; i++) {
                context_qpos_values[n * (NUM_MOTOR + 7) + 7 + mujoco_to_isaaclab[i]] = w0 * motion_sequence->JointPositions(f0)[i] + w1 * motion_sequence->JointPositions(f1)[i];
            }
        }
    }
